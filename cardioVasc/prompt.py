...
def predict_cardiovascular_from_input(model=loaded_model, transformer=loaded_transformer):
    ...
    cholesterol = int(
        input("Enter cholesterol level (1 for normal, 2 for above normal): "))
    glucose = int(
        input("Enter glucose level (1 for normal, 2 for above normal): "))
    smoker = int(input("Do you smoke? (0 for no, 1 for yes): "))
    alcoholic = int(input("Do you drink alcohol? (0 for no, 1 for yes): "))
    bmi = weight / ((height/100)**2)
    active = int(input(
        "Enter activity level (0 for not active, 1 for moderately active, 2 for very active): "))
    bp_category = input(
        "Enter blood pressure category (Normal, Elevated, Hypertension Stage 1, Hypertension Stage 2): ")

    prediction = predict_cardiovascular(age, gender, height, weight, systolic_bp, diastolic_bp,
                                        cholesterol, glucose, smoker, alcoholic, bmi, active, bp_category, model, transformer)
